...
import { saveBeatToCloud } from "@/utils/db";
...
export default function StudioUI({
  ...
  useEffect(() => {
    if (initialPatterns) setPatterns(initialPatterns);
    if (initialBpm) setBpm(initialBpm);
    if (initialTrackUrls) setTrackUrls(initialTrackUrls);
    if (initialVolumes) setVolumes(initialVolumes);
    if (initialMutes) setMutes(initialMutes);
    if (initialBeatName) setBeatName(initialBeatName);
  }, [
    initialPatterns,
    initialBpm,
    initialTrackUrls,
  ...
  const handleShare = async () => {
    ...
    const beatId = await saveBeatToCloud(currentBeat);

    if (beatId) {
      const url = `${window.location.origin}/beat/${beatId}`;
      setShareLink(url);
      navigator.clipboard.writeText(url);
      alert("Beat saved! Link copied to clipboard.");
    }
